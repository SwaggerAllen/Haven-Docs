...
This analysis originally closed with a sketch of the whole-system governance direction it motivated — math-enabled democracy, the people-vote principle, layered authority, the lieutenant pattern. That sketch has since been developed into its own document and is superseded by it. See the [whole-system governance framework](haven-whole-system-governance.md) for the current design; the [organizational structure document](haven-organizational-structure.md) specifies the legal entities that implement it.
