...
> **Audit-campaign marker (2026-06-12).** A pass over the cryptographer review questions (now tracked in the public `haven-open-questions.md` ledger) resolved roughly half of them — some were answerable from the specs or current literature, some were already committed in the architecture document, and several design decisions were made (notably single-signer humanness issuance, replacing the earlier threshold-issuance direction). The genuinely-open questions cluster around the franking compositions and the BBS four-way credential conjunction. **Two consequences for this document, both still pending:** (1) a *freshness pass* on the cryptographic references below — the audit found the field repeatedly ahead of these notes (DMLS/AMT for fork recovery, MlsGov for governance-over-MLS, the 2025 BBS pseudonym/device-binding literature, proactive secret sharing for archive rotation), so reference claims here predate ~2024 and should not be treated as current until refreshed; and (2) a *reconciliation pass* against the architecture document, since at least one review item (the write-cap keypair) was already committed design. Until those passes are done, treat the per-topic “what needs cryptographic review” lists below as superseded by the open-questions ledger where they disagree.
...
1. **Credential-linking integration surface.** The technique (shared-`pid` ZK equality across two BBS credentials, prove a common attribute equal without revealing it) is well-understood — built on textbook Sigma proofs of commitment-equality over the Pedersen/Fiat-Shamir foundations BBS already uses. Different issuer keys don’t break it (same scalar, conjoined verification relations). HIGH confidence on the technique. The open question is the integration surface (MEDIUM confidence): does the adopted CFRG BBS proof API expose cross-credential equality directly, or is it a small hand-composed Sigma conjunction over a shared blinded scalar? *(Resolved by audit 2026-06-12: within a verifier scope, the `bbs-per-verifier-linkability` draft’s pseudonym computation — proven correct against the signed `pid` — means presenting two credentials’ pseudonyms and comparing for equality links them across different issuers through the specified API alone, no hand-composed Sigma needed; this is the draft’s own suggested pattern, and it is Haven’s common case (humanness + decency linked within a community scope). Only linking **outside** any pseudonym scope still needs the hand-composed conjunction below the API. Cross-issuer pseudonym-equality soundness is queued under the composition question. See open-questions ledger CR-B2.)*
