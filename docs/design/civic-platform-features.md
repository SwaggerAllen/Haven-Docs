...
- [Long-term vision, implementation-level detail](#long-term-vision-implementation-level-detail)
- [Phase plan](#phase-plan)
- [Cross-cutting architectural foundations](#cross-cutting-architectural-foundations)
- [1. Identity and accounts](#1-identity-and-accounts)
- [2. Communities](#2-communities)
- [3. Roles and permissions](#3-roles-and-permissions)
- [4. Feeds](#4-feeds)
- [5. Content](#5-content)
- [6. Events and calendars](#6-events-and-calendars)
- [7. Governance](#7-governance)
- [8. Schism](#8-schism)
- [9. Federation](#9-federation)
- [10. Alliances](#10-alliances)
- [11. Registries (specialized alliances)](#11-registries-specialized-alliances)
- [12. Credentials and verification](#12-credentials-and-verification)
- [13. Messaging protocol](#13-messaging-protocol)
- [14. Moderation](#14-moderation)
- [15. Reputation](#15-reputation)
- [16. Templates](#16-templates)
- [17. Discovery and indexability](#17-discovery-and-indexability)
- [18. Archival and retention](#18-archival-and-retention)
- [19. Privacy and metadata](#19-privacy-and-metadata)
- [20. Instance operation](#20-instance-operation)
- [21. Integrations](#21-integrations)
- [22. Constraints](#22-constraints)
- [23. Security properties](#23-security-properties)
- [24. Data model entities](#24-data-model-entities)
- [25. Integration points](#25-integration-points)
- [26. Creator and audience roles](#26-creator-and-audience-roles)
- [27. Payments and external integrations](#27-payments-and-external-integrations)
- [28. Implementation stack](#28-implementation-stack)
- [29. Out of scope (explicitly)](#29-out-of-scope-explicitly)
...
- Disclosure scope is per-vote (or per-proposal-class) governance configuration, not a platform default: a community chooses whether individual votes are attributable or pseudonymous, and whether the recount worksheet is members-only or public. Default is members-only. The scope is fixed and visible before voting opens and cannot be widened retroactively (enforced cryptographically — what was published members-only is out of reach to re-publish wider, not merely flagged); loosening disclosure for *future* votes is itself a governance change subject to the asymmetric thresholds in Section 7.7 [Phase 2]
- Recount worksheet: for any vote, the tally authority can publish the set of per-decision pseudonyms (and, for cardinality-capped decisions, the pseudonym→count multiset) at the configured disclosure scope, committed to the verifiable log, so any eligible recipient can independently recount (total = distinct pseudonyms) and confirm their own ballot is included by re-deriving their per-decision pseudonym. This makes the single tally authority checkable rather than trusted; publishing it adds no cross-scope linkability beyond what the credential unlinkability model already assumes [Phase 2]
...
- Foundation Alliance specifics (developed in the whole-system-governance document): the Foundation Alliance uses the same membership requirements and join flow as any other alliance — no privileged admission path — which keeps it under the same sybil-resistance machinery and makes it competable-with (a rival alliance can form with the same mechanisms; Foundation authority rests on codebase stewardship and recognition, not protocol privilege). Instance alliances are ineligible for Foundation Alliance membership (the humanness issuer must not hold weight denominated in its own credential). Anti-inflation direction for Foundation-level counting (working, settles by end of Phase 2): the counting population is anchored to attested in-person event attendance (Section 6.5) rather than raw credential count, supported by issuance-rate transparency, member sampling, and tenure-vesting of governance weight [direction; not a shipped feature until validated]
...
- Specific committing AEAD primitive, key-reveal granularity, and composition with MLS PrivateMessages and sender-key broadcast layer are pending cryptographer review. (Per the June 2026 audit: the MLS-PrivateMessage composition builds on published governance-over-MLS work — MlsGov, S&P 2024 — and franks underneath MLS’s own AEAD; the sender-key broadcast-layer franking has no published prior art and is the genuinely novel construction, since per-message commitments pull against the broadcast layer’s key-amortization; transcript franking follows Namavari & Ristenpart 2025 and provides contiguity/non-omission assurance distinct from transport ordering.)
...
- Cross-community (alliance-pool) juries serve two purposes beyond capacity: they keep a community that has drifted from quietly ceasing to police itself (external jurors do not share the drift), and they provide an unentangled decider for disputes between people who know each other (the recusal / change-of-venue case). Configurable by case type — routine cases internal, conflict-of-interest cases and appeals to the allied pool.
