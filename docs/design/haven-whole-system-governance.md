...
- [Purpose](#purpose)
- [The fundamental approach: math-enabled democracy](#the-fundamental-approach-math-enabled-democracy)
- [The people-vote principle](#the-people-vote-principle)
- [Layered authority](#layered-authority)
  - [Platform identity layer](#platform-identity-layer)
  - [Foundation Alliance composition criteria](#foundation-alliance-composition-criteria)
  - [Foundation Alliance composition mechanics](#foundation-alliance-composition-mechanics)
  - [Subsystem layer (lieutenants)](#subsystem-layer-lieutenants)
  - [Instance layer](#instance-layer)
  - [Community layer](#community-layer)
  - [Alliance layer](#alliance-layer)
  - [Authority interactions](#authority-interactions)
- [The lieutenant pattern in detail](#the-lieutenant-pattern-in-detail)
  - [Contribution attestations](#contribution-attestations)
  - [Confirmation process](#confirmation-process)
  - [Lieutenant authority](#lieutenant-authority)
  - [Lieutenant removal](#lieutenant-removal)
  - [What lieutenants don’t do](#what-lieutenants-dont-do)
- [Bootstrap: the path from founder control to functioning governance](#bootstrap-the-path-from-founder-control-to-functioning-governance)
  - [Phased growth and the constitutional convention](#phased-growth-and-the-constitutional-convention)
  - [Constitutional entrenchment](#constitutional-entrenchment)
  - [The honest acknowledgment](#the-honest-acknowledgment)
  - [Initial Foundation Alliance composition](#initial-foundation-alliance-composition)
  - [Initial lieutenants](#initial-lieutenants)
  - [The single-share Foundation Alliance period](#the-single-share-foundation-alliance-period)
- [How this binds against the founder-binding failure patterns](#how-this-binds-against-the-founder-binding-failure-patterns)
...
**A recurring pattern: constitutional pre-commitment.** Several mechanisms across the platform share one shape — a body decides the rule for handling a future stress *while trust is still intact and no one yet knows which side they will be on*, and the rule then executes when the stressful moment arrives rather than being renegotiated under pressure. This is the political-theory idea of constitutional pre-commitment (Elster’s *Ulysses and the Sirens* binding-oneself line; related to Rawls’s veil of ignorance for the not-knowing-your-side part), not a Haven invention. It shows up in: disclosure scope fixed before a vote opens and not retroactively widened; external-auditor access pre-defined in a vote’s configuration and dormant until invoked; the convention’s counting rules declared in advance with no grandfathering; and pre-committed conditional failover (a community pre-authorizing, behind multisig, an automated cut-over to an allied replica under configured circumstances). The common virtue is that the decision is made when judgment is unclouded and binding, so the moment of conflict executes a prior agreement instead of fighting over the rules and the stakes at once. Where this pattern appears, the binding should be genuine — enforced cryptographically or by ratcheted governance thresholds — not a mere policy note a later actor can quietly drop.
...
This means humanness is a *propagated* assertion, not something the platform’s cryptography can manufacture: at the bottom of every humanness credential is an offline human act, and across federation the trust degrades — accepting another party’s humanness credentials means trusting their verification practice, sight unseen. There is no cryptographic fix for this; the mitigations are the attendance-anchoring, rate transparency, and acceptance-policy mechanisms described under Foundation Alliance composition and in the architecture document’s Issuers section. The attendance-anchoring direction matters here specifically because it re-grounds governance counting in something harder to forge than a credential count — physical presence — precisely where the propagated-trust floor is weakest.
...
**A community speaks through a single weighted representative.** Each community holds one seat in its alliance, occupied by one representative, and that seat carries weight proportional to the deduplicated humans behind it. The community fills the seat by whatever internal process it already uses — Haven does not run elections inside communities or require any particular internal democracy; the representative simply speaks for the community. This is a deliberate choice to avoid imposing new democratic plumbing on bodies that already govern themselves, and it is also what keeps representative bodies within the size an MLS group can support (a chamber cannot seat every individual directly). The honest cost is that a single representative casts the community’s whole weighted bloc, so an internal minority within a community is not separately visible at the alliance level — a community split 60/40 internally presents as its representative’s position. This trades internal-minority representation for institutional autonomy and protocol simplicity; the trade is deliberate because “communities are real and run themselves” is a core commitment, and surfacing internal splits at the platform level would require the platform to see inside each community’s own vote, which it declines to do. Whether to let communities *optionally* seat more than one weighted representative to express internal pluralism is an open design question (below).
...
There is a second, sharper reason for this exclusion: the instance alliance is the *humanness issuer*. If it could also hold Foundation Alliance governance weight, the issuer of the credential would directly hold weight denominated in the very credential it issues — the most direct form of the circular-trust-root problem, where an issuer can mint governance weight for itself. The ineligibility rule breaks that loop structurally: the path to Foundation weight must run through a real social alliance (affiliation, not hosting), so an issuer can mint credentials but cannot convert them to its own weight without a socially-grounded community vouching it in.
...
**No privileged admission path.** Foundation Alliance membership uses the same requirements and join flow as any other alliance. There is no bespoke admission mechanism. This is a deliberate security property, not just consistency: it guarantees the Foundation Alliance is covered by exactly the same sybil-resistance machinery as every ordinary alliance — the in-person gate, locality-grounded vouching, and the attendance-anchoring direction below — so there is no separate, privileged door to secure or to attack. It forbids ever giving the Foundation Alliance a special-case membership mechanism, however convenient.

**The Foundation Alliance is therefore competable-with.** Because no mechanism is privileged, nothing prevents a rival alliance from forming with the same structure, the same governance math, and the same sybil resistance, and competing for legitimacy. The Foundation Alliance’s authority does not come from protocol privilege — it comes from stewardship of the canonical codebase (the lieutenants are elected over it) and from accumulated recognition by communities and relying parties. A competitor *may* form, but to displace the Foundation Alliance it must fork the entire code-and-maintenance apparatus and rebuild legitimacy from nothing: expensive, not impossible. This resolves the apparent tension between “competable-with” and “authoritative” — the contested prize is the codebase and recognition, not the membership flow. It is the same anti-capture principle as first-class schism, lifted to the alliance layer. Stated as a thesis: **the platform makes communities forkable (schism), the trust anchor forkable (the competable Foundation Alliance), and the founder bound by succession — three layers, one principle: exit is always possible, never free.** This is the governance-layer expression of the capture-resistance the founder-binding analysis is built around.

**Anti-inflation direction (working, not settled).** Because the threshold is denominated in member count and humanness is self-issued, raw credential-counting is inflatable by a lax or captured issuer. The direction under development anchors the counting population to *attested in-person event attendance* rather than to raw credential holding: an entity’s count toward the threshold is its members with attested attendance at some number of in-person events over a trailing window. Events with attendance verification are already a core content type, so honest communities pay nothing new; the work falls on organizers running attendance verification, not on members. This converts a sybil attack from key-minting into operating real humans through real events for a sustained period — the same cost wall real institutions face (entryism, congregation-packing), consistent with the platform’s match-real-institutions philosophy. Supporting mechanisms: issuance-rate transparency (anomalous issuance is publicly visible in the verifiable logs), statistical sampling of a claiming entity’s members, and tenure-vesting of governance weight (mass fresh issuance carries no immediate weight, forcing every fast attack into the slow, exposed version). The denominator must use the same attested count as the numerator, or the scheme leaks. The honest residual: a patient, funded adversary operating real humans through real events over years can still buy weight — bounded by tenure vesting, the two-thirds thresholds, and the public rate logs. Unlinkable proof of “attended ≥N attested events” across communities is an open construction question on the cryptographer review list. The fairness cost — homebound, disabled, and dispersed members discounted from governance weight — is real and shares the problem with every attendance-based institution; a vouched reduced-weight non-attendance path is a candidate relief valve, not yet designed.
...
One alliance function carries weight against capture and drift: **shared jury pools.** Allied communities can agree to draw moderation jurors from each other’s membership, so a reported case in one community is decided partly by people bound by shared alliance rules but external to that community. This is a structural check the community cannot provide for itself. A community that has drifted toward an extreme stops policing its own boundary — its own jury shares the drift — and shared jury pools restore consequences from outside the drift without any higher layer overriding the community’s scope. (It is also the practical relief for the hardest part of small-community moderation, deciding cases between people who know each other; see the architecture document’s moderation section.) Like other alliance functions it is opt-in and configurable; its anti-drift value sits alongside the competability of the Foundation Alliance as a second forkability-flavored check — exit and external accountability rather than central override.
...
**Schism handling for Foundation Alliance.** If Foundation Alliance representatives disagree fundamentally, can the Foundation itself schism? What does that mean legally and cryptographically? (The competability commitment above is part of the answer at the cryptographic/legitimacy layer — a dissenting bloc can in principle form a rival alliance — but the legal questions around the 501(c)(3) and the canonical codebase stewardship are unresolved and interact with the founder-binding structure.) **Cross-credential interaction.** Different institutions may issue overlapping credentials. How does the framework handle cases where someone has credentials from multiple sources that grant different voting weight?

**Contested-vote auditing.** For the small fraction of votes whose legitimacy is genuinely disputed (a contested leadership election, a schism vote, a Foundation-threshold claim), cryptographic verifiability persuades the satisfied but not the aggrieved — the loser of a disputed vote wants an accountable human to check, not a proof. Real election practice has the pattern: accredited observers and risk-limiting audits, with auditor access defined *in advance* and dormant until invoked. The design requirement this places on configurable votes is that any external-auditor escalation path be pre-committed in the vote’s configuration before ballots are cast, never negotiated after a dispute (when trust is already too broken to negotiate). This is one of three independent places the design arrives at bounded, consented, independent human oversight as the tier above cryptographic verification (alongside ethnographic observation and sybil-sampling) — see the independent observation layer notebook.

**Single weighted delegate vs. proportional internal seating.** The current design gives each community one seat carrying weight proportional to its constituency, filled by the community’s own internal process (above). The alternative — letting a community’s seats split to reflect its *internal* vote distribution, so an internal 60/40 surfaces as 60/40 at the alliance level — would represent internal minorities but requires the platform to know each community’s internal vote (invasive, breaking “communities run themselves,” or gameable via self-report) and multiplies seats against the MLS chamber-size ceiling. The single-delegate choice is the current commitment; a possible middle path is letting communities *optionally* seat multiple weighted representatives to express internal pluralism if they choose, without the platform mandating any visibility into their process. The relevant prior art for implementation (not justification) is the apportionment literature — mapping a weighted population onto bounded seats while preserving proportionality, with its known paradoxes (the methods of D’Hondt, Sainte-Laguë, Huntington-Hill, and the rounding/quota paradoxes they trade among) being the same problems any weighted-seat scheme inherits.

**Attendance-anchoring amplifies turnout capture.** One-member-one-vote’s classic failure mode is turnout/quorum capture — a small dedicated faction that always shows up controls low-turnout decisions. Anchoring governance weight to attested attendance *amplifies* this, because it structurally rewards exactly the people who show up. This is partly a deliberate feature, not only a bug: rewarding in-person participation is mission-aligned (encouraging embodied community is the project’s whole point), so the design intends attendance to matter. The open question is how to keep the mission-aligned benefit (attendance carries weight) without the pathology (a small dedicated faction dominating low-turnout decisions) — a real interaction between two of the platform’s own mechanisms that needs an explicit answer (quorum floors, participation-rate normalization, or similar) before attendance-anchoring is finalized.
