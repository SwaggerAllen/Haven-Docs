...
This is design documentation, not a formal specification. It reflects current settled direction. Items still in active design are clearly marked. A note on process: this documentation was drafted with substantial AI assistance. The architecture, the decisions, and the mistakes are the author’s. Companion documents cover the [whole-system governance framework](haven-whole-system-governance.md), [organizational structure](haven-organizational-structure.md), [federation readiness commitments](civic-platform-federation-readiness.md), [implementation-level feature specifications](civic-platform-features.md)
...
The two-layer alliance composition (MLS governance with sender-key messaging) uses established protocols but the composition needs external cryptographic review before implementation begins. The message franking design has been mapped well enough to bring focused questions to franking authors. Verifiable log infrastructure adopts C2SP tlog-witness for witness coordination and the Sunlight/TesseraCT temporal-sharding pattern; specific Rust implementation path is pending investigation. Content schema format is JSON Schema with Haven conventions on top.
...
- Specific threshold signature scheme (multi-sig vs FROST-style aggregation)
- Committing AEAD primitive for franking; specific composition with MLS PrivateMessages
- Key-reveal granularity for franking escalation; composition with MLS forward secrecy
- Multi-layer franking verifier key delegation structure
- BBS composition: blind-committed `pid` supporting selective disclosure plus per-verifier pseudonyms plus nullifier soundness simultaneously, plus device-authorization as a bound value in the proof (rather than a separate pseudonym keypair)
- Credential-linking through shared-`pid` ZK equality: the technique is well-understood; the open question is the integration surface in the adopted CFRG BBS proof API (direct support vs hand-composed Sigma conjunction)
- Archive-key construction for institutional archive (threshold encryption vs proxy re-encryption; archivist-set rotation without whole-archive re-encryption); onboarding context does not need this primitive
- Blind-issuance properties for humanness credential issuer (instance alliance) and home-community decency issuers — confirm participating issuer cannot derive or forge existing member’s pseudonym
- Per-epoch write-capability keypair construction and rotation (for role-write enforcement)
- McMillion partition derivation: epoch-in-Context rationale (understand whether redundant or deliberate explicit-binding)
- State reconciliation protocol for operator-hostile recovery
- Fork-recovery state retention: minimum state and window needed to detect and reconcile a fork while preserving as much forward secrecy as possible (incident-scoped, not standing)
