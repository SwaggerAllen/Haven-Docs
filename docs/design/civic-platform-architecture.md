...
- [Core entities](#core-entities)
  ...
  - [Issuers](#issuers)
  - [Haven Foundation](#haven-foundation)
  - [Strutco](#strutco)
- [Cryptographic architecture](#cryptographic-architecture)
  - [Identity](#identity)
  - [The two-layer alliance design](#the-two-layer-alliance-design)
  - [Threshold multi-signature for alliance authority](#threshold-multi-signature-for-alliance-authority)
  - [Anonymous credentials](#anonymous-credentials)
  - [Message franking for verifiable reporting](#message-franking-for-verifiable-reporting)
  - [Verifiable logs](#verifiable-logs)
  - [Key derivation and capture-forward discipline](#key-derivation-and-capture-forward-discipline)
  - [Confidential sub-groups and the role-write enforcement model](#confidential-sub-groups-and-the-role-write-enforcement-model)
  - [Signed state and event sourcing](#signed-state-and-event-sourcing)
- [Content model](#content-model)
  - [Schema format](#schema-format)
  - [Feeds defined by allowed content types](#feeds-defined-by-allowed-content-types)
  - [Interface-based composition for client rendering](#interface-based-composition-for-client-rendering)
  - [History and retention](#history-and-retention)
  - [Recovery and the forward secrecy carve-out](#recovery-and-the-forward-secrecy-carve-out)
- [Identity, reputation, and credentials](#identity-reputation-and-credentials)
  - [Sliding-scale pseudonymity](#sliding-scale-pseudonymity)
  - [Universal vs. contextual reputation](#universal-vs-contextual-reputation)
  - [Vouching](#vouching)
  - [Credential ecosystem](#credential-ecosystem)
  - [Verification levels are not visible within a community](#verification-levels-are-not-visible-within-a-community)
- [Governance](#governance)
  - [Multi-level structure](#multi-level-structure)
  - [Math-enabled democracy](#math-enabled-democracy)
  - [Foundation Alliance composition](#foundation-alliance-composition)
  - [Configurable governance modules](#configurable-governance-modules)
  - [Temporal governance mechanisms](#temporal-governance-mechanisms)
  - [Schism](#schism)
- [Moderation](#moderation)
  - [Multi-level rule-based moderation](#multi-level-rule-based-moderation)
  - [Shared jury pools across alliances](#shared-jury-pools-across-alliances)
  - [Rules apply prospectively](#rules-apply-prospectively)
  - [CSAM as universal prohibition](#csam-as-universal-prohibition)
  - [Verification via franking](#verification-via-franking)
  - [Guardian role for youth participation](#guardian-role-for-youth-participation)
  - [What the platform does not do](#what-the-platform-does-not-do)
  - [Structural friction against criminal use](#structural-friction-against-criminal-use)
- [Federation](#federation-1)
  - [Direct federation model](#direct-federation-model)
  - [Hub instance pattern for alliances](#hub-instance-pattern-for-alliances)
  - [Instance identity](#instance-identity)
  - [Mailbox addressing](#mailbox-addressing)
  - [Partition keys for routing](#partition-keys-for-routing)
  - [The blind hub](#the-blind-hub)
  - [Metadata residuals and why they are accepted](#metadata-residuals-and-why-they-are-accepted)
  - [Cross-mailbox references](#cross-mailbox-references)
  - [Three-tier replication](#three-tier-replication)
  - [Operator transitions and fork recovery](#operator-transitions-and-fork-recovery)
  - [Replica servers for public content scaling](#replica-servers-for-public-content-scaling)
  - [Public verifiable logs](#public-verifiable-logs)
  - [Defederation](#defederation)
- [Home community and migration](#home-community-and-migration)
  - [Home community](#home-community)
  - [Community migration](#community-migration)
- [Locality and discovery](#locality-and-discovery)
  - [In-person trust as default](#in-person-trust-as-default)
  - [Locality registries](#locality-registries)
  - [Privacy in discovery](#privacy-in-discovery)
- [Revenue model](#revenue-model)
  - [Minimal community dues](#minimal-community-dues)
  - [Physical credential production (primary revenue)](#physical-credential-production-primary-revenue)
  - [Instance operator revenue share](#instance-operator-revenue-share)
  - [Payment processing without platform cut](#payment-processing-without-platform-cut)
  - [What the model excludes](#what-the-model-excludes)
  - [The structural floor: bulletin-board vs. attention-auction](#the-structural-floor-bulletin-board-vs-attention-auction)
- [Tickets, good standing, and credential-gated access](#tickets-good-standing-and-credential-gated-access)
  - [Sybil-resistant ticketing](#sybil-resistant-ticketing)
  - [Good standing through credential issuance](#good-standing-through-credential-issuance)
- [Creator and audience roles within communities](#creator-and-audience-roles-within-communities)
- [Plugin system](#plugin-system)
- [Substrate and implementation](#substrate-and-implementation)
  - [Implementation stack](#implementation-stack)
  - [Cryptographic protocol design status](#cryptographic-protocol-design-status)
- [What’s currently in active design](#whats-currently-in-active-design)
  - [Cryptographic implementation decisions pending review](#cryptographic-implementation-decisions-pending-review)
  - [Architectural directions in development](#architectural-directions-in-development)
  - [Library and infrastructure investigations](#library-and-infrastructure-investigations)
  - [Working group tracking](#working-group-tracking)
  - [Documentation and presentation](#documentation-and-presentation)
- [Phase 6: late protocol additions](#phase-6-late-protocol-additions)
- [Open questions](#open-questions)
- [Distinctive design principles (summary)](#distinctive-design-principles-summary)
...
Explicit mutual-aid pacts between communities. Alliances are group entities with their own governance, cryptographic identity, and content. Alliances can share moderation decisions, shunning lists, credential trust, and event calendars. They can also share *adjudication*, not just decisions: an allied community’s members can be empaneled into a community’s moderation jury, so the people deciding a case are bound by shared alliance rules but external to the reporting community. This shared-jury-pool capability is a distinctive alliance function — it answers two problems a self-contained community can’t solve for itself. A community that has drifted toward an extreme polices its own boundary less because its jury shares the drift; drawing jurors from allied communities that don’t share it means a report still carries consequences. And the hardest part of volunteer moderation — adjudicating a dispute between people you personally know — is relieved when the juror can be an allied member bound by the same rules but not entangled with the parties (the outside-arbiter logic of a change of venue). Because it weakens the rule-ownership benefit (a community judged by its own people), jury sharing is opt-in and configurable, typically by case type: routine cases stay internal where local-norm fidelity matters most, while conflict-of-interest cases and appeals can route to an allied pool where neutrality matters most. Shared cross-community adjudication is only possible where communities are first-class federated entities with portable credentials and shared rules — it cannot be approximated by bots or plugins on a centralized platform, where a community cannot empanel jurors from another. Alliances are distinct from federation — federation is between instances, alliance is between communities. Alliances can themselves be members of other alliances, supporting hierarchical structures.
...
Reference implementations (Sunlight from Filippo Valsorda / Let’s Encrypt, TesseraCT from Google) implement the Static CT API and serve as design references. The mature Static-CT ecosystem is written in Go and consolidated around the Tessera library in 2025; there is no comparably mature Rust implementation. Rather than reimplement in Rust to match the cryptographic core, Haven runs a Go Static-CT log (Tessera/Sunlight) as a separate service alongside the Rust core — the log has a clean HTTP boundary, so the Go stays quarantined behind the Static-CT API rather than threaded through the application. This adds a third language to the stack (Elixir/Phoenix backend, Rust crypto core, Go log), an accepted cost; the discipline is to run the upstream log unmodified as a service and treat any fork of it as a real maintenance liability rather than a free customization, since use-case-specific forking would re-incur most of the maintenance cost a full port would have. Operating a log at Haven’s volumes is far below the WebPKI scale these tools target (a six-month WebPKI shard reaches ~1.5 TB; Haven’s logs are a small fraction of that).
...
### Shared jury pools across alliances

Adjudication of a report can draw jurors from allied communities, not only from the reporting community itself (see Alliances). This is configurable, typically by case type — routine cases stay internal, where fidelity to local norms matters most, while conflict-of-interest cases and appeals can route to an allied pool, where neutrality matters most. Shared jury pools serve two structural purposes a self-contained community cannot serve for itself: they keep a community that has drifted toward an extreme from quietly ceasing to police its own boundary (its own jury would share the drift; allied jurors do not), and they relieve the hardest part of small-community moderation — adjudicating disputes between people who know each other — by allowing an unentangled allied juror to decide, on the logic of a change of venue. The cost is a partial reduction in rule-ownership, which is why it is opt-in; the capability is only possible because communities are first-class federated entities with portable credentials and shared rules.
...
### Metadata residuals and why they are accepted

The blind hub cannot read content or learn group rosters — that is property one, content and membership confidentiality, and Haven provides it. It does not provide property two, traffic-analysis or correlation resistance: an operator can still observe activity volume, timing, and which devices are active under a partition key, and a determined operator could attempt to correlate this across rotating partition keys. Partition-key rotation raises the cost of sustained correlation but does not make it impossible. Haven states this plainly rather than implying a privacy it does not deliver, because a system that *implies* correlation-resistance it lacks is worse than one that is honest about the residual — users who misjudge their exposure is the harm to avoid.

The reason this residual is acceptable is that Haven does not rest the user’s protection on correlation being *impossible*. It rests on correlation, even when successful, buying an attacker little and costing them a great deal. The correlation attack is only the first of several barriers, and the later ones are not cryptographic:

- **Technical friction.** Partition keys and anonymous credentials convert an impulsive action into deliberate, sustained, technical effort.
- **Social friction.** An operator acting on correlated metadata is overriding a visible democratic decision, often against people they know personally — a non-cryptographic barrier the attack cannot dissolve.
- **Exit cost.** Schism and identity-intact migration mean a targeted community can leave with itself intact, so the attack fails its own goal and costs the operator standing and members.
- **Scale economics.** Continuously re-correlating every community on a large instance across rotating partition keys is expensive per community and continuous — it does not scale to mass surveillance.

This is the opposite of privacy theater. Theater is when the appearance of protection substitutes for protection; here the protection is real but located in the consequences and economics of the attack rather than in cryptographic impossibility, and the honest disclosure of the residual is what keeps it from misleading anyone. There is also a Haven-specific reason the stakes of defeating the blind hub are higher than in a generic messenger: because governance decisions are committed to MLS state, the encrypted state *is* the governance record. An operator who defeats it has not merely read messages — it has visibly attacked the integrity of the democratic substrate, an act exposed by the transparency logs and the representative reconciliation process. The cost of breaking governance-bearing encrypted state is therefore far above the generic-messaging case, which is part of why the residual is defensible in Haven’s context specifically.
...
Because a registry is an alliance, joining one follows the same in-person grounding as joining any alliance: a member of the joining community and a member of an existing registry-member community exchange community-handshake QR codes in person, and the registry ratifies through its governance. There is no separate discovery, push, or pull mechanism for registries beyond browsing their public listings. This means a community can *find* a registry by browsing but still has to make in-person contact to *join* it — and there is currently no in-band channel for initiating that contact, since a non-member cannot post into a registry’s feeds, so first contact is out-of-band by default. This friction is deliberate and load-bearing rather than an oversight: it is part of what prevents a single large registry from frictionlessly accreting the whole network. Any contact mechanism eventually built must preserve that friction and must not make registry-joining one-click, which would recreate the default-to-the-largest-provider gravity that re-centralizes nominally decentralized networks.

For the same anti-consolidation reason, any directory or registry listing that orders communities or operators presents them in a randomized or locality-based order by default, never ranked by size or popularity. Popularity-ranked discovery is a centralizing force — it sends each new entrant to the largest existing option — and Haven’s discovery surfaces are designed to resist that gravity rather than amplify it.
...
- Go Static-CT log integration (run Tessera/Sunlight as a service alongside the Rust core; deployment, ops, and the discipline of using upstream unmodified rather than forking)
...
Cashed out from a community’s side, the principles amount to four guarantees that hold even against the operator: the operator cannot read your members’ messages; you can move the whole community to another host with members and identities intact; no one can delete or sell the community out from under you; and no one is mining your members’ attention. Some of these exist elsewhere on their own — end-to-end encryption most obviously — but the whole set at once, held structurally rather than by the operator’s grace, is what no incumbent offers. “Ownership” on other platforms is permission — branding, data export, admin rights, all revocable by the operator who grants them. These four hold whether or not the operator consents.
